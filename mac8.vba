Sub AggregateDataWithDecadaAndSilent()
    ...
    If srcWs Is Nothing Or decWs Is Nothing Or silWs Is Nothing Or volWs Is Nothing Then
        ...
        ' Восстановление настроек перед выходом
    ...
    ' Словари для кэширования диапазонов строк Уровня 1 и Уровня 2 под формулы аплинкинга
    ...
    ' Кэширование ключей для связки строк в цикле форматирования
    Dim rowKeysDict As Object
    Set rowKeysDict = CreateObject("Scripting.Dictionary")
    ...
    For Each key In dict.Keys
        ...
        ' Смещение +3 с учетом будущей двухэтажной шапки
        lvl1StartRows(idx1) = headerIdx + 3 
        rowKeysDict(headerIdx + 3) = "L1_" & idx1
        ...
        For j = 2 To UBound(decArr, 1)
            ...
            If decKey = key Then
                ...
                tempRow(3) = decArr(j, 5) ' Трудозатраты план
                tempRow(4) = ""           ' План (формула SUMIFS/SUM)
                tempRow(5) = ""           ' Факт (формула SUMIFS/SUM)
                tempRow(6) = ""           ' Дельта объемов (формула)
                tempRow(7) = ""           ' % откл-ия (формула)
                tempRow(8) = ""           ' Остаток объемов (формула)
                tempRow(9) = curDateH     ' Начало работ
                tempRow(10) = curDateI    ' Конец работ
                tempRow(11) = decArr(j, 9)' Раб. дни
                For k = 12 To 13: tempRow(k) = "": Next k
                rowsColl.Add tempRow
                endLvl1 = rowsColl.Count
                alignRightColl.Add endLvl1 
                
                Dim lvl2CurRow As Long
                lvl2CurRow = rowsColl.Count + 3
                lvl2StartRows(idx1 & "_" & idx2) = lvl2CurRow
                rowKeysDict(lvl2CurRow) = "L2_" & idx1 & "_" & idx2
                ...
                If silDict.Exists(silKey) Then
                    ...
                    For itemIdx = 1 To silRowsItems.Count
                        ...
                        tempRow(2) = matchSil(0) ' Технологический шифр в графу B
                        tempRow(3) = ""          ' Ед.изм (графа C)
                        tempRow(4) = ""          ' План (графа D)
                        tempRow(5) = ""          ' Факт (графа E)
                        tempRow(6) = ""          ' Дельта объемов -> F (формула)
                        tempRow(7) = ""          ' % откл-ия -> G (формула)
                        tempRow(8) = ""          ' Остаток объемов -> H
                        tempRow(9) = matchSil(3) ' Начало
                        tempRow(10) = matchSil(4)' Конец
                        tempRow(11) = ""         ' Раб. дни
                        For k = 12 To 13: tempRow(k) = "": Next k
                        ...
                        If volDict.Exists(volKey) Then
                            ...
                            ' matchVol: 0=План, 1=Ед.изм, 2=Факт, 3=Остаток, 4=Статус, 5=% готовности
                            ...
                            tempRow(13) = matchVol(5) ' Процент готовности -> M (под батарейку)
                        End If
                    ...
                    lvl2EndRows(idx1 & "_" & idx2) = rowsColl.Count + 3
                Else
                    lvl2EndRows(idx1 & "_" & idx2) = lvl2CurRow
                End If
            End If
        Next j
        
        lvl1EndRows(idx1) = rowsColl.Count + 3 
        ...
        End If
    Next key
    ' 6. Перенос коллекции в результирующий массив (13 колонок под C:O в памяти)
    ...
    With newWs.Range("C1:O1")
        ...
        .Interior.Color = RGB(20, 20, 20) ' Матовый глубокий черный
    ...
    With newWs.Range("C2:O3")
        ...
        .Font.Bold = True
        .Font.Color = RGB(255, 255, 255)
        .Interior.Color = RGB(45, 45, 45) ' Графитовый темный
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
        .WrapText = True
    End With
    ...
    ' ЖЕСТКАЯ ЗАЩИТА ОТ ЗАПЯТЫХ: Задаем текстовый формат ячейкам ДО выгрузки данных
    newWs.Range("C4:C" & (rowsColl.Count + 3)).NumberFormat = "@"
    
    ' Выгрузка основного массива данных начиная с ячейки C4
    newWs.Range("C4").Resize(rowsColl.Count, 13).Value = outArr
    
    ' СДВИГ СТРУКТУРЫ ВЛЕВО (Сбриваем А и В)
    newWs.Columns("A:B").Delete Shift:=xlToLeft
    
    ' ТЕПЕРЬ КООРДИНАТЫ ЛИСТА (ОТ А ДО М):
    ' А=№, B=Шифр, C=Трудозатраты, D=План, E=Факт, F=Дельта, G=% откл, H=Остаток, I=Начало, J=Конец, K=Дни, L=Статус, M=Процент
    
    ' Сразу превентивно защищаем новый результирующий столбец А текстовым форматом
    newWs.Range("A4:A" & (rowsColl.Count + 3)).NumberFormat = "@"
    
    Dim currRow As Long, dotsCount As Long, curType As String, curKey As Variant
    ...
    For i = 1 To rowsColl.Count
        ...
        If outArr(i, 1) <> "" Then
            ...
            ' Уровень 1: Luxury Deep Black (Матовый черный с белым текстом)
            If dotsCount = 0 Then
                ...
                ' ВНЕДРЕНИЕ ЖИВЫХ ИТОГОВ ДЛЯ УРОВНЯ 1 (Сбор сумм по Уровню 2)
            ...
            ' Уровень 2: Slate Gray (Темно-серый благородный цвет с контрастным БЕЛЫМ текстом)
            ElseIf dotsCount = 1 Then
                curKey = outArr(i, 1)
                subStart = lvl2StartRows(curKey) + 1
                ...
                ' ВНЕДРЕНИЕ ЖИВЫХ ИТОГОВ ДЛЯ УРОВНЯ 2 (Сбор сумм по Уровню 3)
                If subEnd >= subStart Then
            ...
            ' Уровень 3: Детальные строки технологических карт (Шрифт Calibri Жирный)
