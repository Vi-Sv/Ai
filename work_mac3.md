Sub AggregateDataWithDecadaAndSilent()
    ...
    ' Считываем столбцы C:N (C=1, D=2, E=3, G=5, L=10, M=11, N=12)
    ...
    For Each key In dict.Keys
        ...
        ' Уровень 1: Основной шифр (Значение С из VVOD_VOLUM)
        tempRow(1) = idx1
        tempRow(2) = key
        tempRow(3) = dict(key)
        ...
        ' Уровень 2: Детали DECADA (Значение D из VVOD_VOLUM)
        For j = 2 To UBound(decArr, 1)
            ...
            If decKey = key Then
                ...
                If silDict.Exists(silKey) Then
                    ...
                    ' Уровень 3: Детали SILENT_ENGINE (Значение E из VVOD_VOLUM)
                    tempRow(1) = idx1 & "." & idx2 & "." & idx3
                    tempRow(2) = matchSil(0)
                    tempRow(3) = "" ' Дефолтная пустота в графе Е нового листа
                    ...
                    ' МГНОВЕННАЯ ПРОВЕРКА 3-х ПОЗИЦИЙ СРАЗУ (БЕЗ ВТОРОГО ЦИКЛА)
                    ' Собираем точный ключ: Родитель_C | Работа_D _ ТехШифр_E
                    ...
                    If volDict.Exists(volKey) Then
                        ...
                        tempRow(3) = matchVol(0)  ' G -> падает в графу Е нового листа (индекс 3)
                        tempRow(9) = matchVol(1)  ' L -> падает в графу K нового листа (индекс 9)
                        tempRow(11) = matchVol(2) ' M -> падает в графу M нового листа (индекс 11)
                        tempRow(12) = matchVol(3) ' N -> падает в графу N нового листа (индекс 12)
                    End If
