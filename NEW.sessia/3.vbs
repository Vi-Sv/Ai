Sub AggregateDataWithDecadaAndSilent()
    ...
    Next i
    ' =========================================================================
    ' БЛОК 5: ЖЕСТКИЙ ДВУХПРОХОДНОЙ РАСЧЕТ ФОРМУЛ И ИЕРАРХИЧЕСКИХ СТАТУСОВ
    ' =========================================================================
    ...
    For r = lastSheetRow To 4 Step -1
        ...
        Else
            ...
            If checkDots = 2 Then
                ...
                ' Оптимизация: скрываем -100.00% при нулевом факте, чтобы разгрузить отчет визуально
    ...
    ' Второй проход: сверху вниз с динамическими статусами (Расчет Уровня 1)
        ...
        If levelStr <> "" Then
            ...
            If checkDots = 0 Then
                ...
                If subEnd >= subStart Then
                    ...
                    newWs.Cells(r, "I").Formula = "=IF(COUNTIF(I" & subStart & ":I" & subEnd & ", ""не по плану"")>0,""не по плану"",IF(ROUND(J" & r & ",4)>=1,""Работы завершены"",IF(J" & r & ">0,""Ведутся работы"",""Работы не начаты"")))"
            ...
            ElseIf checkDots = 1 Then
                If newWs.Cells(r, "D").Formula <> "" Then
                    newWs.Cells(r, "I").Formula = "=IF(AND(ISNUMBER(D" & r & "),ISNUMBER(E" & r & "),E" & r & ">D" & r & "),""не по плану"",IF(ROUND(J" & r & ",4)>=1,""Работы завершены"",IF(J" & r & ">0,""Ведутся работы"",""Работы не начаты"")))"
                End If
                
            ' Уровень 3
            ElseIf checkDots = 2 Then
                newWs.Cells(r, "I").Formula = "=IF(AND(ISNUMBER(D" & r & "),ISNUMBER(E" & r & "),E" & r & ">D" & r & "),""не по плану"",IF(ROUND(J" & r & ",4)>=1,""Работы завершены"",IF(J" & r & ">0,""Ведутся работы"",IF(TRIM(I" & r & ")="""",""Работы не начаты"",I" & r & "))))"
            End If
        End If
    Next r
