' ======================================================================================
' ПОЛНАЯ КОРРЕКТНАЯ ВЕРСИЯ: ИЕРАРХИЯ С ДВУХКРИТЕРИАЛЬНОЙ СОРТИРОВКОЙ, СУММАМИ И КОДАМИ
' ======================================================================================
Option Explicit
...
Sub BuildHierarchyTree()
    ...
    ' Загрузка листов в Variant-массивы в память
...
' БЛОК 2 ИЗ 4: ПОСТРОЕНИЕ ИЕРАРХИЧЕСКОЙ СТРУКТУРЫ С ЗАЩИТОЙ ОТ ОШИБОК EXCEL
    ...
    ' 1. Индексация листа VVOD_VOLUM по ID (с защитой от ошибок Excel)
    ...
    ' 2. Индексация листа SILENT_ENGINE по ID (с защитой от ошибок Excel)
    ...
    ' 3. Индексация листа DECADA по тексту из графы D (Приведение к нижнему регистру + Trim)
    For i = 2 To UBound(decData, 1)
        keyText = decData(i, 4) ' Графа D
        If Not IsError(keyText) Then
            If Not IsEmpty(keyText) And keyText <> "" Then
                keyText = Trim(LCase(CStr(keyText)))
                dictDecada(keyText) = decData(i, 2) ' Столбец B
            End If
        End If
    Next i

    ' 4. Построение дерева связей с наложением ДВОЙНОГО INNER JOIN и расчетом кумулятивных сумм
    For i = 5 To UBound(srcData, 1)
        ...
        If Not IsError(keyID) And Not IsError(valL1) And Not IsError(valL2) And Not IsError(valL3) Then
            If Not IsEmpty(valL1) And valL1 <> "" And dictVol.Exists(keyID) And dictEng.Exists(keyID) Then
                ' Инициализация мета-словаря для Уровня 1 (с добавлением Set во избежание ошибки 450)
                ...
                ' Инициализация мета-словаря для Уровня 2
                ...
                ' Расчет остатка для Уровня 3 (если факт больше плана, пишем 0)
                ...
                extraData(8) = dEnd
                extraData(9) = valL3
                extraData(10) = valRem        ' Очищенный Остаток
                
                dictL3(keyID) = extraData
            End If
        End If
    Next i
' БЛОК 3 ИЗ 4: ГЛОБАЛЬНАЯ СОРТИРОВКА ПО Ч/Ч, РАСЧЕТ ПРОЦЕНТА ГОТОВНОСТИ (MAX 100%) И ВЫГРУЗКА
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            remL1 = metaL1("SrcVol") - metaL1("Fact")
            If remL1 < 0# Then remL1 = 0#
            ...
            wsNew.Cells(outRow, 9).Value = remL1
            wsNew.Cells(outRow, 10).Value = metaL1("Spent")
            
            ' Расчет процента готовности для Уровня 1 с ограничением в 100% (max 1.0)
            If metaL1("SrcVol") > 0 Then
                pL1 = metaL1("Fact") / metaL1("SrcVol")
                If pL1 > 1# Then pL1 = 1#
            ...
            ' НАСТРОЙКА ЦВЕТА: Фирменный глубокий матовый оттенок корпуса «Синий Титан» (iPhone Blue Titanium)
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    remL2 = metaL2("SrcVol") - metaL2("Fact")
                    If remL2 < 0# Then remL2 = 0#
                    ...
                    wsNew.Cells(outRow, 9).Value = remL2
                    ...
                    ' Расчет процента готовности для Уровня 2 с ограничением в 100% (max 1.0)
                    If metaL2("SrcVol") > 0 Then
                        pL2 = metaL2("Fact") / metaL2("SrcVol")
                        If pL2 > 1# Then pL2 = 1#
                    ...
                    For Each kVol In subDictL3.Keys
                        ...
                        ' Расчет процента готовности для Уровня 3 с ограничением в 100% (max 1.0)
                        ...
                        If currentExtra(7) <> 0 And currentExtra(7) <> "00.01.1900" Then
                            wsNew.Cells(outRow, 11).Value = currentExtra(7)
                        End If
                        
                        If currentExtra(8) <> 0 And currentExtra(8) <> "00.01.1900" Then
                            wsNew.Cells(outRow, 12).Value = currentExtra(8)
                        End If
                        
                        ' Нанесение границ Уровня 3 с расширением диапазона со столбца B до M
                    ...
                    ' Физическая группировка Уровня 3 под Уровнем 2
        ...
        Next SortRow
    End If
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, НАСТРОЙКА РАЗМЕРОВ И ГРАФИЧЕСКИХ ИНДИКАТОРОВ (ТЕМНО-СИНИЙ СТИЛЬ)
    ...
    ' Нанесение общей сетки тонких границ на всю заполненную таблицу со 2-й строки (от столбца B до M)
    ...
    ' Повторное выделение контуров Уровня 3 жирной рамкой (расширено со столбца B до M)
    ...
    ' Настройка шапки во 2-й строке (Высота 70 по ТЗ, Центрирование + перенос текста от B до M)
    With wsNew.Range("B2:M2")
    ...
    ' Форматирование нового столбца B («Код DECADA»)
    With wsNew.Columns("B")
        .ColumnWidth = 16
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
        .WrapText = True
    ...
    ' Удаляем старые правила условного форматирования в столбце M во избежание наслоения
    rngPercent.FormatConditions.Delete
    
    ' Внедрение графической шкалы (DataBar) с фиксированными границами без двоеточий
    ...
    MsgBox "Макрос полностью выполнен. Отрицательные остатки убраны, лимит 100% внедрен, стиль изменен на темно-синий.", vbInformation
End Sub
