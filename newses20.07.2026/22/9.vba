...
' БЛОК 3 ИЗ 5 (ЧАСТЬ 1): ФОРМИРОВАНИЕ ШАПКИ, СТРУКТУРНАЯ СОРТИРОВКА И ВЫГРУЗКА УРОВНЯ 1
    ...
    ' Формирование жестких заголовков на строках 2 и 3
    ...
    wsNew.Cells(2, 12).Value = "Планируемая дата завершения"
    wsNew.Cells(2, 13).Value = "Процент готовности"
    
    ' Объединение боковых столбцов по вертикали (строки 2 и 3)
    ...
    ' ==========================================================================
    ' ИСПРАВЛЕНИЕ ВЕРХНЕГО ЯРУСА ШАПКИ (СТРОКА 2) СТРОГО ПО ЭСКИЗУ
    ' ==========================================================================
    ' 1. Группа Плана: объединяет 4 столбца (E, F, G, H)
    wsNew.Cells(2, 5).Value = "Планируемые трудозатраты"
    wsNew.Range("E2:H2").Merge
    
    ' 2. Группа Факта: объединяет 2 столбца (I, J)
    wsNew.Cells(2, 9).Value = "Фактические трудозатраты"
    wsNew.Range("I2:J2").Merge
    
    ' Нижний рабочий ярус (СТРОКА 3) для детальных спецификаций 3 уровня
    wsNew.Cells(3, 5).Value = "Ед. изм."
    wsNew.Cells(3, 6).Value = "Норма на ед."
    wsNew.Cells(3, 7).Value = "Исх. объем"
    wsNew.Cells(3, 8).Value = "Факт. объем"
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            wsNew.Cells(outRow, 9).Value = "Потратили:"
