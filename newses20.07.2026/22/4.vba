...
' БЛОК 4 ИЗ 4 (ЧАСТЬ 1): ВНЕШНЯЯ ГРУППИРОВКА И НАСТРОЙКА ДВУХЪЯРУСНОЙ ШАПКИ
    ...
    ' Динамическое определение внешних границ групп по пустым строкам (начинаем со строки 5, чтобы не задеть шапку)
    currentGroupStart = 4
    ...
    ' Нанесение общей сетки тонких границ на всю заполненную таблицу со 2-й строки (захватывая всю двухъярусную шапку)
    ...
    ' Настройка двухъярусной шапки в строках 2 и 3 (Центрирование + перенос текста от B до M)
    With wsNew.Range("B2:M3")
        ...
        .WrapText = True
    End With
    wsNew.Rows(2).RowHeight = 35 ' Высота верхнего яруса шапки
    wsNew.Rows(3).RowHeight = 35 ' Высота нижнего яруса шапки
       ' БЛОК 4 ИЗ 4 (ЧАСТЬ 2): ГЕОМЕТРИЯ СТОЛБЦОВ, ФОРМАТИРОВАНИЕ УРОВНЕЙ И ТОНКИЕ РАЗДЕЛИТЕЛИ (ВЫСОТА 2)
    
    ' Форматирование столбца B («Код DECADA»)
    ...
    ' ТЗ: Ширина графы D ровно 53, перенос текста включен
    With wsNew.Columns("D")
        .ColumnWidth = 53
        .WrapText = True
    End With
    
    ' Форматирование столбца E
    With wsNew.Columns("E")
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
    End With
    
    ' Форматирование столбцов F, G, H, I, J (Ширина 11, Центрирование)
    ...
    ' НАСТРОЙКА УРОВНЕЙ ПО ТЗ (Шрифт Segoe UI, ВЕЗДЕ ЖИРНЫЙ, РАЗМЕР СТРОГО 11)
    Dim cellMark As Variant
    For i = 4 To totalRows
        cellMark = wsNew.Cells(i, 3).Value
        
        ' Если строка пустая (разделитель групп)
        If cellMark = "" Then
            wsNew.Rows(i).RowHeight = 2 ' ТЗ: Тонкая полоса-разделитель высотой в 2 единицы
        Else
            ' Для всех рабочих строк накладываем базовые правила
            ...
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 13)).Font.Size = 11 ' ТЗ: Весь текст теперь строго 11 размера
            wsNew.Rows(i).RowHeight = 48 ' Высота информационных строк строго 48
            ...
            If InStr(1, cellMark, ".") = 0 Then
                ...
                ' Текст в графы D строго по центру
                wsNew.Cells(i, 4).HorizontalAlignment = xlCenter
                wsNew.Cells(i, 4).VerticalAlignment = xlCenter
                
                ' ТЗ: Ячейку «Цель на трудозатраты:» центрируем по горизонтали и вертикали
            ...
            ElseIf UBound(Split(cellMark, ".")) = 1 Then
                ...
                ' Текст в графе D влево по центру
            ...
            Else
                ' ==========================================================================
                ' УРОВЕНЬ 3 (Белые рабочие строки)
                ' ==========================================================================
                ...
                ' Формат с копейками для физических объемов на рабочем уровне
    ...
    MsgBox "Сборка полностью завершена! Уровень 2 приведен к эталонному виду, формулы ч/ч пересчитаны.", vbInformation
    ' === МАРКЕР_ФИНАЛЬНОГО_ОФОРМЛЕНИЯ_END ===
        ' === МАРКЕР_ГЕНЕРАЦИИ_ТИТУЛЬНОГО_ЗАГЛОВКА_START ===
    Dim titleCell As Range
    Set titleCell = wsNew.Range("B1:M1")
    
    ' 1. Физическое объединение диапазона B1:M1 по вашему ТЗ
    titleCell.Merge
    
    ' 2. Запись многострочного текста титула строго по ТЗ
    ...
    ' 3. Применение стиля по ТЗ (Times New Roman, 14pt, Жирный, Выравнивание по центру)
    With titleCell
        ...
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
        .WrapText = True ' Включаем автоматический перенос по строкам
        
        ' Стираем сетку границ вокруг титула, чтобы он смотрелся аккуратным блоком сверху таблицы
        .Borders.LineStyle = xlNone
    End With
    
    ' 4. Жесткое ТЗ: Изменяем высоту имеющейся первой строчки строго до 120 единиц
