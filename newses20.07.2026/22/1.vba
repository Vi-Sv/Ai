' ======================================================================================
' ПОЛНАЯ ВЕРСИЯ: ДВУХУРОВНЕВАЯ ШАПКА, АГРЕГАЦИЯ ВЕСОВ Ч/Ч И ПРЕМИУМ ДИЗАЙН «СИНИЙ ТИТАН»
' ======================================================================================
Option Explicit
    ...
    For i = 5 To UBound(srcData, 1)
        ...
        If Not IsError(keyID) And Not IsError(valL1) And Not IsError(valL2) And Not IsError(valL3) Then
            If Not IsEmpty(valL1) And valL1 <> "" And dictVol.Exists(keyID) And dictEng.Exists(keyID) Then
                ...
                ' ВОЗВРАТ ИСХОДНОЙ ЛОГИКИ: Считываем Исх. объем строго из столбца K (11) по ТЗ
                valSrcVol = 0#: If IsNumeric(srcData(i, 11)) Then valSrcVol = CDbl(srcData(i, 11))
                valNorm = 0#: If IsNumeric(srcData(i, 13)) Then valNorm = CDbl(srcData(i, 13))  ' Столбец M (13)
                ...
                ' Точный расчет трудозатрат ч/ч на основе ТЗ: Объем (K) * Норма (M)
                itemPlanHours = valSrcVol * valNorm
                itemFactHours = valFact * valNorm
                
                ' Накопление сумм на Уровень 2
                ...
                extraData(1) = keyID
                extraData(2) = srcData(i, 12) ' Ед. изм. (Столбец L)
                extraData(3) = valSrcVol      ' Исходный объем (Столбец K)
                extraData(4) = valNorm        ' Норма на ед. (Столбец M)
...
' БЛОК 3 ИЗ 4 (ЧАСТЬ 1): ФОРМИРОВАНИЕ ШАПКИ, СТРУКТУРНАЯ СОРТИРОВКА И ВЫГРУЗКА УРОВНЯ 1
    ...
    ' --- ФОРМИРОВАНИЕ ДВУХЪЯРУСНОЙ ШАПКИ ТАБЛИЦЫ (СТРОКИ 2 И 3) ---
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    ' Расчет процента готовности для Уровня 2 с ограничением в 100%
...
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, НАСТРОЙКА РАЗМЕРОВ, ДВУХЪЯРУСНОЙ ШАПКИ И БАТАРЕЕК
    ...
    ' Установка жирного шрифта для всей заполненной таблицы и всей шапки
    ...
    ' Форматирование столбца E (Центрирование ед. изм. и надписи «Цель на трудозатраты:»)
    ...
    ' Внимание: Для Уровня 1 объединенные ячейки H-I автоматически унаследуют этот числовой формат
    ...
    MsgBox "Архитектурное обновление завершено! Двухъярусная шапка сформирована, веса ч/ч посчитаны в фоне.", vbInformation
End Sub
