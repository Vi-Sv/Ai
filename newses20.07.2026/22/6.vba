...
' БЛОК 4 ИЗ 4: ГРУППИРОВКА, ТОТАЛЬНОЕ ОФОРМЛЕНИЕ, МОНОЛИТНЫЕ ПЛАШКИ И ТИТУЛ С KPI-БЛОКОМ
    ...
    ' 3. Нанесение общей сетки тонких границ на всю заполненную таблицу со 2-й строки
    ...
    ' 4. Повторное выделение контуров Уровня 3 жирной рамкой
    ...
    ' 5. Настройка двухъярусной шапки в строках 2 и 3
    ...
    For i = 4 To totalRows
        ...
        Else
            ...
            wsNew.Rows(i).RowHeight = 48 ' ТЗ: Высота всех информационных строк строго 48
                ...
                ' ТЗ: Текст прижимаем вправо, цифры влево — для создания эффекта слитности
                wsNew.Cells(i, 5).HorizontalAlignment = xlRight: wsNew.Cells(i, 5).VerticalAlignment = xlCenter ' Текст цели
                wsNew.Cells(i, 8).HorizontalAlignment = xlLeft:  wsNew.Cells(i, 8).VerticalAlignment = xlCenter  ' Число плана ч/ч
                wsNew.Cells(i, 9).HorizontalAlignment = xlRight: wsNew.Cells(i, 9).VerticalAlignment = xlCenter ' Текст "Затраты:"
                wsNew.Cells(i, 10).HorizontalAlignment = xlLeft: wsNew.Cells(i, 10).VerticalAlignment = xlCenter ' Число факта ч/ч
                
                ' ТЗ: Стираем внутренние штрихи в плашке E-F-G
                ...
                ' ТЗ: Стираем вертикальную границу МЕЖДУ текстом и числом, рождая идеальную строчку
                wsNew.Cells(i, 8).Borders(xlEdgeLeft).LineStyle = xlNone  ' Линия между G и H
                wsNew.Cells(i, 10).Borders(xlEdgeLeft).LineStyle = xlNone ' Линия между I—J
