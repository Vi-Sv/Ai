...
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, НАСТРОЙКА РАЗМЕРОВ, ДВУХЪЯРУСНОЙ ШАПКИ И БАТАРЕЕК
    ...
    ' Форматирование столбцов F, G, H, I, J (Ширина 10, Числовой формат с разделителями, Центрирование)
    ...
    ' Накладываем красивый числовой формат с разделителями разрядов
    ' Для Уровня 1 и Уровня 2 объединенные ячейки H-I отобразят ч/ч без копеек
    For i = 4 To totalRows
        ' Если строка — Уровень 3 (Элемент), то ставим формат с копейками для объемов
        If InStr(1, wsNew.Cells(i, 3).Value, ".") <> InStrRev(wsNew.Cells(i, 3).Value, ".") Then
            wsNew.Range(wsNew.Cells(i, 6), wsNew.Cells(i, 10)).NumberFormat = "#,##0.00"
        Else
            ' Для Уровня 1 и 2 ставим целые числа для ч/ч
