...
' БЛОК 3 ИЗ 4: ГЛОБАЛЬНАЯ СОРТИРОВКА ПО ТРУДОЗАТРАТАМ (Ч/Ч) ДЛЯ ВСЕХ УРОВНЕЙ И ВЫГРУЗКА
    ...
    ' === СОРТИРОВКА ЭТАП 1: ИСПРАВЛЕНО — ГЛАВНЫЕ ГРУППЫ УРОВНЯ 1 СОРТИРУЮТСЯ СНАЧАЛА ПО Ч/Ч ===
    If dictL1.Count > 0 Then
        ...
        ' Пузырьковая сортировка Уровня 1 по ч/ч и датам
        For SortRow = 1 To UBound(arrL1) - 1
            For SortCol = SortRow + 1 To UBound(arrL1)
                ...
                swapNeeded = False
                ' Правило 1: Объект без ч/ч (0), а следующий с ч/ч (>0) -> двигаем объект с ч/ч наверх
                If spent1 = 0 And spent2 > 0 Then
                    swapNeeded = True
                ' Правило 2: У обоих Объектов есть ч/ч -> сортируем Объекты по УБЫВАНИЮ ч/ч
                ElseIf spent1 > 0 And spent2 > 0 Then
                    If spent1 < spent2 Then swapNeeded = True
                ' Правило 3: У обоих Объектов НЕТ ч/ч -> сортируем Объекты по хронологии дат (МинСтарт)
                ElseIf spent1 = 0 And spent2 = 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            ' === СОРТИРОВКА ЭТАП 2: УРОВНИ 2 ПО Ч/Ч ВНУТРИ ТЕКУЩЕГО УРОВНЯ 1 ===
...
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, НАСТРОЙКА РАЗМЕРОВ И ВИЗУАЛЬНОГО ОФОРМЛЕНИЯ К, L
    ...
    MsgBox "Двухкритериальная сортировка по трудозатратам ч/ч и календарный расчет выполнены успешно.", vbInformation
End Sub
