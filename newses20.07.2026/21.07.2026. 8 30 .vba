...
Sub BuildHierarchyTree()
    ...
    Set wsDec = ThisWorkbook.Worksheets("DECADA") ' Четвертый лист-справочник совпадений
    On Error GoTo 0
    
    ' Проверка существования всех необходимых справочников
    ...
    lastRowDec = wsDec.Cells(wsDec.Rows.Count, "D").End(xlUp).Row ' Поиск последней строки по графе D
    ...
    ' Загрузка листов в оперативную память
    ...
    decData = wsDec.Range("A1:D" & lastRowDec).Value ' Забираем диапазон от А до D
...
' БЛОК 2 ИЗ 4: ПОСТРОЕНИЕ ИЕРАРХИЧЕСКОЙ СТРУКТУРЫ И ИНДЕКСАЦИЯ СПРАВОЧНИКОВ
    ...
    Next i
        ...
        ' 3. Индексация листа DECADA по тексту из графы D (Приведение ключа к нижнему регистру + Trim)
    For i = 2 To UBound(decData, 1)
        keyText = decData(i, 4) ' Графа D
        If Not IsEmpty(keyText) And keyText <> "" Then
            ' Очищаем от пробелов и принудительно переводим в НИЖНИЙ регистр
            keyText = Trim(LCase(CStr(keyText)))
            ' Записываем значение из столбца B в индекс словаря
...
' БЛОК 3 ИЗ 4: ГЛОБАЛЬНАЯ СОРТИРОВКА ПО Ч/Ч, РАСЧЕТ ПРОЦЕНТА ГОТОВНОСТИ И ВЫГРУЗКА С ОГРАНИЧЕНИЕМ ПОИСКА КОДОВ (ТОЛЬКО УРОВЕНЬ 2)
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            ' ИСПРАВЛЕНИЕ: Для Уровня 1 проверка по словарю DECADA полностью удалена (строка останется пустой)
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    ' ЖЕСТКОЕ ПРАВИЛО: Сопоставление с листом DECADA выполняется ТОЛЬКО для Уровня 2
                    ...
                    For Each kVol In subDictL3.Keys
                        ...
                        ' ИСПРАВЛЕНИЕ: Для Уровня 3 проверка по словарю DECADA полностью удалена (строка останется пустой)
...
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, ГРАФИЧЕСКИЕ ИНДИКАТОРЫ И СХЛОПЫВАНИЕ (С УЧЕТОМ ГРАФЫ B)
    ...
    ' Нанесение общей сетки тонких границ на всю заполненную таблицу со 2-й строки (теперь от столбца B до M)
    ...
    MsgBox "Макрос полностью завершен. Данные с листа DECADA успешно сопоставлены в столбце B.", vbInformation
End Sub
