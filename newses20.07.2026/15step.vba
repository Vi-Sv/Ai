...
' БЛОК 2 ИЗ 4: ПОСТРОЕНИЕ ДЕРЕВА, КАЛЕНДАРНАЯ АГРЕГАЦИЯ И КУМУЛЯТИВНЫЙ РАСЧЕТ СУММ
    ...
    Dim valSrcVol As Double, valFact As Double
    ...
    For i = 5 To UBound(srcData, 1)
        ...
        If Not IsEmpty(valL1) And valL1 <> "" And dictVol.Exists(keyID) And dictEng.Exists(keyID) Then
            ' Инициализация мета-словаря для Уровня 1 (добавлены счетчики объемов)
            ...
            ' Инициализация мета-словаря для Уровня 2 (добавлены счетчики объемов)
            ...
            ' Извлечение и программная очистка числовых значений 3-го уровня
            valSrcVol = 0#: If IsNumeric(srcData(i, 11)) Then valSrcVol = CDbl(srcData(i, 11)) ' Столбец K (Исх. объем)
            valFact = 0#: If IsNumeric(dictVol(keyID)(1)) Then valFact = CDbl(dictVol(keyID)(1))   ' С листа объемов (Факт)
            
            ' Накапливаем кумулятивные суммы на Уровень 2
            l2Meta("SrcVol") = l2Meta("SrcVol") + valSrcVol
            l2Meta("Fact") = l2Meta("Fact") + valFact
            
            ' Накапливаем кумулятивные суммы на Уровень 1
            ...
            ' Формирование массива элемента Уровня 3 (Размерность увеличена до 10 для Остатка)
            ReDim extraData(1 To 10)
            extraData(1) = keyID
            extraData(2) = srcData(i, 12) ' Столбец L (Ед. изм.)
            extraData(3) = valSrcVol      ' Исх. объем (число)
            extraData(4) = srcData(i, 13) ' Столбец M (Норма на ед.)
            extraData(5) = valFact        ' Факт (число)
            extraData(6) = dictVol(keyID)(2) ' Потрачено ч/ч
            extraData(7) = dStart
            extraData(8) = dEnd
            extraData(9) = valL3
            extraData(10) = valSrcVol - valFact ' Математический расчет остатка для Уровня 3
...
' БЛОК 3 ИЗ 4: СКОРРЕКТИРОВАННАЯ СОРТИРОВКА ВСЕХ УРОВНЕЙ И ВЫГРУЗКА ИЕРАРХИИ С РАССЧИТАННЫМИ СУММАМИ
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            ' ВЫГРУЗКА УРОВНЯ 1 (Вывод рассчитанных сумм и разности для Объекта)
            wsNew.Cells(outRow, 3).Value = CStr(idxL1)
            wsNew.Cells(outRow, 4).Value = k1
            wsNew.Cells(outRow, 7).Value = metaL1("SrcVol")                  ' Сумма Исх. объем (Уровень 1)
            wsNew.Cells(outRow, 8).Value = metaL1("Fact")                    ' Сумма Факт (Уровень 1)
            wsNew.Cells(outRow, 9).Value = metaL1("SrcVol") - metaL1("Fact") ' Вычисленный Остаток (Уровень 1)
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    ' ВЫГРУЗКА УРОВНЯ 2 (Вывод рассчитанных сумм и разности для Подгруппы)
                    wsNew.Cells(outRow, 3).Value = idxL1 & "." & idxL2
                    wsNew.Cells(outRow, 4).Value = "    " & k2
                    wsNew.Cells(outRow, 7).Value = metaL2("SrcVol")                  ' Сумма Исх. объем (Уровень 2)
                    wsNew.Cells(outRow, 8).Value = metaL2("Fact")                    ' Сумма Факт (Уровень 2)
                    wsNew.Cells(outRow, 9).Value = metaL2("SrcVol") - metaL2("Fact") ' Вычисленный Остаток (Уровень 2)
                    ...
                    ' ВЫГРУЗКА УРОВНЯ 3 (Элементы)
                    For Each kVol In subDictL3.Keys
                        idxL3 = idxL3 + 1
                        currentExtra = subDictL3(kVol)
                        
                        wsNew.Cells(outRow, 3).Value = idxL1 & "." & idxL2 & "." & idxL3
                        wsNew.Cells(outRow, 4).Value = currentExtra(9)  ' Наименование
                        wsNew.Cells(outRow, 1).Value = currentExtra(1)  ' ID в столбце А
                        wsNew.Cells(outRow, 5).Value = currentExtra(2)  ' Ед. изм.
                        wsNew.Cells(outRow, 6).Value = currentExtra(4)  ' Норма на ед.
                        wsNew.Cells(outRow, 7).Value = currentExtra(3)  ' Исх. объем (Уровень 3)
                        wsNew.Cells(outRow, 8).Value = currentExtra(5)  ' Факт (Уровень 3)
                        wsNew.Cells(outRow, 9).Value = currentExtra(10) ' Вычисленный Остаток (Уровень 3)
                        wsNew.Cells(outRow, 10).Value = currentExtra(6) ' Потрачено ч/ч
...
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, НАСТРОЙКА РАЗМЕРОВ И ВИЗУАЛЬНОГО ОФОРМЛЕНИЯ К, L
    ...
    MsgBox "Расчет сумм объемов и остатков для уровней 1, 2 и 3 успешно завершен. Структура выгружена.", vbInformation
End Sub
