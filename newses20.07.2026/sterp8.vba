        ...
        If Not IsEmpty(valL1) And valL1 <> "" And dictVol.Exists(keyID) Then
            ...
            If Not IsEmpty(valL2) And valL2 <> "" Then
                ...
                If Not IsEmpty(valL3) And valL3 <> "" Then
                    ...
                    extraData(5) = dictVol(keyID)(1) ' Из J
                    extraData(6) = dictVol(keyID)(2) ' Из O
...
' БЛОК 3 ИЗ 4: ЗАПИСЬ ОБНОВЛЕННОЙ ШАПКИ, СТРУКТУРЫ С УЧЕТОМ ПУСТОЙ ГРАФЫ I ДЛЯ ДАННЫХ
    ...
    ' Запись названий колонок во вторую строку (I — пустое поле «Остаток объем»)
    ...
    ' Обход дерева в памяти и построчная выгрузка (ячейки столбца I остаются пустыми)
    For Each k1 In dictL1.Keys
        ...
        Set rngL1 = wsNew.Range(wsNew.Cells(outRow, 3), wsNew.Cells(outRow, 10))
        ...
        For Each k2 In dictL2.Keys
            ...
            Set rngL2 = wsNew.Range(wsNew.Cells(outRow, 3), wsNew.Cells(outRow, 10))
            ...
            For Each k3 In dictL3.Keys
                ...
                ' Столбец I (ячейка wsNew.Cells(outRow, 9)) сознательно пропускается пустым
...
' БЛОК 4 ИЗ 4: ВНЕШНЯЯ ГРУППИРОВКА, НАСТРОЙКА ФОРМАТА ЧИСЕЛ F, G, H, I, J И СХЛОПЫВАНИЕ
    ...
    wsNew.Range("A2:J" & totalRows).Font.Bold = True
    
    ' Настройка шапки во 2-й строке (Центрирование + перенос текста)
    With wsNew.Range("C2:J2")
    ...
    MsgBox "Структурированный лист сформирован. Столбец I оставлен пустым под формулы.", vbInformation
End Sub
