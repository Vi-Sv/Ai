...
' БЛОК 3 ИЗ 5 (ЧАСТЬ 1): ФОРМИРОВАНИЕ ШАПКИ, СТРУКТУРНАЯ СОРТИРОВКА И ВЫГРУЗКА УРОВНЯ 1
    ...
    ' Исправление: Заносим текст Остатка объема в верхний ярус строки 2, чтобы он не затирался при Merge
    wsNew.Cells(2, 11).Value = "Остаток объем"
    wsNew.Cells(2, 12).Value = "Запланированная дата начала работ"
    wsNew.Cells(2, 13).Value = "План на конец работ"
    wsNew.Cells(2, 14).Value = "Процент готовности"
    ...
    ' Названия нижнего яруса шапки (столбец K (11) убран отсюда в строку 2)
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            ' Вычисление процента готовности Объекта (L1) через ВЕСА трудозатрат ч/ч (столбец N)
            If metaL1("TargetHours") > 0 Then
                pL1 = metaL1("DoneHours") / metaL1("TargetHours")
                If pL1 > 1# Then pL1 = 1#
                wsNew.Cells(outRow, 14).Value = pL1
            Else
                wsNew.Cells(outRow, 14).Value = 0#
            End If
            
            ' ИСПРАВЛЕНИЕ: Запись нуля вместо пустоты для Уровня 1, чтобы сработал формат с минусом "-"
            If metaL1("MinStart") <> DateAdd("yyyy", 100, Date) Then wsNew.Cells(outRow, 12).Value = metaL1("MinStart") Else wsNew.Cells(outRow, 12).Value = 0
            If metaL1("MaxEnd") <> CDate(0) Then wsNew.Cells(outRow, 13).Value = metaL1("MaxEnd") Else wsNew.Cells(outRow, 13).Value = 0
            
            wsNew.Rows(outRow).RowHeight = 30
            Set rngL1 = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 14))
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    ' Вычисление процента готовности подгруппы (L2) через ВЕСА трудозатрат ч/ч (столбец N)
                    ...
                    ' ИСПРАВЛЕНИЕ: Запись нуля вместо пустоты для Уровня 2, чтобы сработал формат с минусом "-"
                    If metaL2("StartDate") <> 0 Then wsNew.Cells(outRow, 12).Value = metaL2("StartDate") Else wsNew.Cells(outRow, 12).Value = 0
                    If metaL2("EndDate") <> 0 Then wsNew.Cells(outRow, 13).Value = metaL2("EndDate") Else wsNew.Cells(outRow, 13).Value = 0
                    
                    Set rngL2 = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 14))
                    ...
                    For Each kVol In subDictL3.Keys
                        ...
                        wsNew.Cells(outRow, 7).Value = currentExtra(3)  ' Исх. объем
                        
                        ' ОЖИВЛЕНИЕ БРЕШИ: Автоматический расчет плановых ч/ч (Норма F * Исх. объем G)
                        ...
                        ' ЖЕСТКОЕ ИСПРАВЛЕНИЕ: Проверка валидности дат через IsDate для Уровня 3
                        ' Если дата реальная и не пустая — пишем её, иначе пишем числовой 0 для маски
                        If IsDate(currentExtra(7)) And currentExtra(7) <> 0 And currentExtra(7) <> "00.01.1900" Then
                            wsNew.Cells(outRow, 12).Value = CDate(currentExtra(7))
                        Else
                            wsNew.Cells(outRow, 12).Value = 0
                        End If
                        
                        If IsDate(currentExtra(8)) And currentExtra(8) <> 0 And currentExtra(8) <> "00.01.1900" Then
                            wsNew.Cells(outRow, 13).Value = CDate(currentExtra(8))
                        Else
                            wsNew.Cells(outRow, 13).Value = 0
                        End If
                        
                        ' Процент готовности уходит в столбец N (14)
                        If currentExtra(3) > 0 Then
                            pL3 = currentExtra(5) / currentExtra(3)
                            If pL3 > 1# Then pL3 = 1#
                            wsNew.Cells(outRow, 14).Value = pL3
                        Else
                            wsNew.Cells(outRow, 14).Value = 0#
                        End If
                        
                        Set rngL3 = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 14))
            ...
            If SortRow <= UBound(arrL1) Then
                ...
                Set rngBlankRow = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 14))
...
' БЛОК 4 ИЗ 5 (ЧАСТЬ 1): ВНЕШНЯЯ ГРУППИРОВКА И ОБЩИЙ КОНТУРНЫЙ ОБХОД ДЛЯ УРОВНЯ 3 ПО НОВОЙ СЕТКЕ
    ...
    Dim totalSpentHours As Double ' Новое: переменная для сбора общего факта ч/ч
    ...
    totalTargetHours = 0#
    totalSpentHours = 0# ' Инициализация
    
    If dictL1.Count > 0 Then
        For Each k1Var In dictL1.Keys
            totalTargetHours = totalTargetHours + dictL1(k1Var)("TargetHours")
            totalSpentHours = totalSpentHours + dictL1(k1Var)("Spent") ' Собираем факт
        Next k1Var
    End If
    ...
    ' 3. Нанесение общей базовой сетки тонких границ на всю заполненную таблицу со 2-й строки (расширено до N)
    Dim wholeTable As Range
    Set wholeTable = wsNew.Range("B2:N" & totalRows)
    wholeTable.Borders.LineStyle = xlContinuous
    wholeTable.Borders.Weight = xlThin
    
    ' 4. ИСПРАВЛЕНИЕ: Поиск пачки строк Уровня 3 и обводка ВСЕГО БЛОКА в жирную рамку по новым координатам (B:N)
    ...
    For i = 4 To totalRows
        ...
        Else
            If blockStartRow <> 0 Then
                wsNew.Range(wsNew.Cells(blockStartRow, 2), wsNew.Cells(i - 1, 14)).BorderAround LineStyle:=xlContinuous, Weight:=xlMedium
                blockStartRow = 0
            End If
        End If
    Next i
    ...
    If blockStartRow <> 0 Then
        wsNew.Range(wsNew.Cells(blockStartRow, 2), wsNew.Cells(totalRows, 14)).BorderAround LineStyle:=xlContinuous, Weight:=xlMedium
    End If
    
    ' 5. Настройка двухъярусной шапки в строках 2 и 3 (расширено до N)
    With wsNew.Range("B2:N3")
        .Font.Name = "Segoe UI"
        .Font.Size = 10
        .Font.Bold = True
    ...
    ' ТЗ: Точечно переопределяем ширину графы I на 15 единиц для визуального простора
    wsNew.Columns("I").ColumnWidth = 15
    
        ' ТЗ: Точечно переопределяем ширину графы J на 15 единиц
    ...
    For i = 4 To totalRows
        ...
        Else
            ' Для всех заполненных ячеек устанавливаем жирный Segoe UI 11 на весь рабочий диапазон (B:N)
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Font.Name = "Segoe UI"
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Font.Bold = True
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Font.Size = 11
            ...
            If InStr(1, cellMark, ".") = 0 Then
                ...
                ' ==========================================================================
                wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Font.Italic = True
                ...
                With wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Borders(xlEdgeBottom)
                ...
                ' ИСПРАВЛЕНИЕ: Проверяем реальные часы в графе H (8)
                valPlanHoursH = Val(wsNew.Cells(i, 8).Value)
                If valPlanHoursH = 0 Then
                    wsNew.Cells(i, 14).NumberFormat = "0.0%;;""-"""
                Else
                    wsNew.Cells(i, 14).NumberFormat = "0.0%"
                End If
            ...
            Else
                ...
                ' ==========================================================================
                wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Font.Italic = False
                wsNew.Cells(i, 4).HorizontalAlignment = xlLeft: wsNew.Cells(i, 4).VerticalAlignment = xlCenter
                ...
                wsNew.Cells(i, 10).NumberFormat = "#,##0;;""-"""
                wsNew.Cells(i, 11).NumberFormat = "#,##0.00;;""-"""
                
                ' Для Уровня 3 сохраняем строгую двухфакторную проверку по физическим объемам G (7) и I (9)
    ...
    ' 8. Форматирование сместившихся столбцов дат L и M с маскировкой нулевых дат минусом
    With wsNew.Range("L:M")
        .ColumnWidth = 18
    ...
    ' 9. --- НАСТРОЙКА И ВИЗУАЛИЗАЦИЯ СТОЛБЦА N («БАТАРЕЙКИ» ПРОЦЕНТОВ) ---
    Dim rngPercent As Range
    Set rngPercent = wsNew.Range("N4:N" & totalRows)
    
    With wsNew.Columns("N")
        .ColumnWidth = 16
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
    End With
    
    ' Градиентная заливка батареек накладывается поверх наших интеллектуальных форматов
    ...
    ' ==========================================================================
    ' 10. ОФОРМЛЕНИЕ ИМЕЮЩЕЙСЯ СТРОКИ 1 ПОД ТИТУЛ С KPI-БЛОКОМ (B:N)
    ' ==========================================================================
    ...
    Set titleTextCell = wsNew.Range("B1:L1") ' Левый текстовый блок расширен до L из-за ротации
    Set titleSumCell = wsNew.Range("M1:N1")  ' Правый KPI-блок ч/ч занимает M-N
    ...
    ' ТЗ: Вывод симметричного KPI (План и Факт друг под другом через перенос строки)
    wsNew.Cells(1, 13).Value = "ПЛАН ТРУДОЗАТРАТ: " & Format(totalTargetHours, "#,##0") & " ч/ч" & vbCrLf & vbCrLf & _
    ...
    ' Строгие двойные бухгалтерские линии под расширенным титульным блоком (B:N)

    ' Строгие двойные бухгалтерские линии под расширенным титульным блоком (B:N)
    wsNew.Range("B1:N1").Borders(xlEdgeBottom).LineStyle = xlDouble
    wsNew.Range("B1:N1").Borders(xlEdgeBottom).Weight = xlThick
    ...
    MsgBox "Генерация отчета завершена! Все ТЗ по ротации, закрытию бреши, ширине I (15) и KPI выполнены.", vbInformation
