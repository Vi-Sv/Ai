...
' БЛОК 3 ИЗ 5 (ЧАСТЬ 1): ФОРМИРОВАНИЕ ШАПКИ, СТРУКТУРНАЯ СОРТИРОВКА И ВЫГРУЗКА УРОВНЯ 1
    ...
    ' Сдвигаем даты и проценты вправо из-за ротации столбцов и бреши плана ч/ч
    ...
    ' ИСПРАВЛЕНИЕ: Добавлено вертикальное объединение диапазона K2:K3 для Остатка объема
    ...
    ' Шапка первого яруса симметрично группирует Plan/Fact по новому эскизу
    ...
    ' Названия нижнего яруса шапки под новую ротацию и появление бреши плана ч/ч
    ...
    wsNew.Cells(3, 11).Value = "Остаток объем"
    ...
    For i = 4 To totalRows
        ...
        Else
            ...
            If InStr(1, cellMark, ".") = 0 Then
                ...
                ' Направленное встречное выравнивание для создания эффекта безрамочной слитности
                ...
                ' Стираем внутренние штрихи в плашке E-F-G
                ...
                ' Стираем вертикальные границы на стыках Текст-Число
                wsNew.Cells(i, 8).Borders(xlEdgeLeft).LineStyle = xlNone
                wsNew.Cells(i, 10).Borders(xlEdgeLeft).LineStyle = xlNone
                
                ' Устранение бага Excel: Жестко прорисовываем нижнюю грань, убирая обрывки
