...
' БЛОК 4 ИЗ 5 (ЧАСТЬ 1): ВНЕШНЯЯ ГРУППИРОВКА И ОБЩИЙ КОНТУРНЫЙ ОБХОД ДЛЯ УРОВНЯ 3 ПО НОВОЙ СЕТКЕ
    ...
    ' 1. ПОДСПОРНЫЙ СЧЕТЧИК: Собираем глобальную сумму плановых ч/ч по всем Объектам 1 уровня для титула
    totalTargetHours = 0#
    If dictL1.Count > 0 Then
    ...
    wsNew.Cells(1, 13).Value = "ПЛАН ТРУДОЗАТРАТ:" & vbCrLf & vbCrLf & Format(totalTargetHours, "#,##0") & " ч/ч"
