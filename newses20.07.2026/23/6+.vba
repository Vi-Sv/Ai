' ======================================================================================
' ИТОГОВАЯ ПРЕМИУМ-ВЕРСИЯ: АВТОНОМНЫЙ ДАШБОРД С РОТАЦИЕЙ ОБЪЕМОВ И РАСЧЕТОМ ПЛАНА Ч/Ч
' ======================================================================================
Option Explicit
...
' БЛОК 3 ИЗ 5 (ЧАСТЬ 1): ФОРМИРОВАНИЕ ШАПКИ, СТРУКТУРНАЯ СОРТИРОВКА И ВЫГРУЗКА УРОВНЯ 1
    ...
    ' ТЗ: Заносим текст Остатка объема и Дельты на верхний ярус строки 2, чтобы они не затирались при Merge
    wsNew.Cells(2, 11).Value = "Остаток объем"
    wsNew.Cells(2, 12).Value = "Дельта"                           ' НОВАЯ ГРАФА ПО ТЗ (Столбец L)
    wsNew.Cells(2, 13).Value = "Запланированная дата начала работ" ' Сдвиг вправо на графу M (13)
    wsNew.Cells(2, 14).Value = "План на конец работ"               ' Сдвиг вправо на графу N (14)
    wsNew.Cells(2, 15).Value = "Процент готовности"                ' Сдвиг вправо на графу O (15)
    
    ' Вертикальное объединение боковых и аналитических столбцов по ярусам 2 и 3
    wsNew.Range("B2:B3").Merge
    wsNew.Range("C2:C3").Merge
    wsNew.Range("D2:D3").Merge
    wsNew.Range("K2:K3").Merge ' Монолитный Остаток объема
    wsNew.Range("L2:L3").Merge ' ТЗ: Монолитная Дельта
    wsNew.Range("M2:M3").Merge ' Монолитная Дата начала
    wsNew.Range("N2:N3").Merge ' Монолитная Дата конца
    wsNew.Range("O2:O3").Merge ' Монолитный Процент прогресса
    
    ' Шапка первого яруса симметрично группирует Plan/Fact
    wsNew.Cells(2, 5).Value = "Планируемые трудозатраты"
    wsNew.Range("E2:H2").Merge
    
    wsNew.Cells(2, 9).Value = "Фактические трудозатраты"
    wsNew.Range("I2:J2").Merge
    
    ' Названия нижнего яруса шапки для детальной спецификации
    wsNew.Cells(3, 5).Value = "Ед. изм."
    wsNew.Cells(3, 6).Value = "Норма на ед."
    wsNew.Cells(3, 7).Value = "Исх. объем"
    wsNew.Cells(3, 8).Value = "План ч/ч"
    wsNew.Cells(3, 9).Value = "Факт. объем"
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            ' Тексты и суммы плановых ч/ч падают строго по координатам безрамочного ТЗ
            wsNew.Cells(outRow, 5).Value = "План трудозатрат на шифр:"
            wsNew.Cells(outRow, 8).Value = metaL1("TargetHours")
            wsNew.Cells(outRow, 9).Value = "Затраты:"
            wsNew.Cells(outRow, 10).Value = metaL1("Spent")
            
            ' Вычисление процента готовности Объекта (L1) через ВЕСА трудозатрат ч/ч (теперь столбец O (15))
            If metaL1("TargetHours") > 0 Then
                pL1 = metaL1("DoneHours") / metaL1("TargetHours")
                If pL1 > 1# Then pL1 = 1#
                wsNew.Cells(outRow, 15).Value = pL1
            Else
                wsNew.Cells(outRow, 15).Value = 0#
            End If
            
            ' Запись нуля вместо пустоты для Уровня 1, чтобы сработал формат с минусом "-" (сдвинуты на 13 и 14)
            If metaL1("MinStart") <> DateAdd("yyyy", 100, Date) Then wsNew.Cells(outRow, 13).Value = metaL1("MinStart") Else wsNew.Cells(outRow, 13).Value = 0
            If metaL1("MaxEnd") <> CDate(0) Then wsNew.Cells(outRow, 14).Value = metaL1("MaxEnd") Else wsNew.Cells(outRow, 14).Value = 0
            
            wsNew.Rows(outRow).RowHeight = 30
            Set rngL1 = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 15)) ' Расширено до O
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    ' Вычисление процента готовности подгруппы (L2) через ВЕСА трудозатрат ч/ч (теперь столбец O (15))
                    If metaL2("TargetHours") > 0 Then
                        pL2 = metaL2("DoneHours") / metaL2("TargetHours")
                        If pL2 > 1# Then pL2 = 1#
                        wsNew.Cells(outRow, 15).Value = pL2
                    Else
                        wsNew.Cells(outRow, 15).Value = 0#
                    End If
                    
                    ' Запись нуля вместо пустоты для Уровня 2, чтобы сработал формат с минусом "-" (сдвинуты на 13 и 14)
                    If metaL2("StartDate") <> 0 Then wsNew.Cells(outRow, 13).Value = metaL2("StartDate") Else wsNew.Cells(outRow, 13).Value = 0
                    If metaL2("EndDate") <> 0 Then wsNew.Cells(outRow, 14).Value = metaL2("EndDate") Else wsNew.Cells(outRow, 14).Value = 0
                    
                    Set rngL2 = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 15)) ' Расширено до O
                    ...
                    ' ==========================================================================
                    ' ВЫГРУЗКА УРОВНЯ 3 (Элементы подгруппы) — РОТАЦИЯ И НОВАЯ АНАЛИТИКА «ДЕЛЬТЫ»
                    ' ==========================================================================
                    For Each kVol In subDictL3.Keys
                        ...
                        wsNew.Cells(outRow, 4).Value = currentExtra(9)  ' Название работы
                        wsNew.Cells(outRow, 1).Value = currentExtra(1)  ' ID
                        wsNew.Cells(outRow, 5).Value = currentExtra(2)  ' Ед. изм.
                        wsNew.Cells(outRow, 6).Value = currentExtra(4)  ' Норма на ед.
                        wsNew.Cells(outRow, 7).Value = currentExtra(3)  ' Исх. объем (G)
                        
                        ' Автоматический расчет плановых ч/ч (Норма F * Исх. объем G)
                        wsNew.Cells(outRow, 8).Value = currentExtra(4) * currentExtra(3)
                        
                        ' РОТАЦИЯ ПО ТЗ: Факт объема переносится в графу I (9)
                        wsNew.Cells(outRow, 9).Value = currentExtra(5)
                        
                        ' Потрачено ч/ч из табеля остается в графе J (10)
                        wsNew.Cells(outRow, 10).Value = currentExtra(6)
                        
                        ' РОТАЦИЯ ПО ТЗ: Остаток объема переносится в графу K (11) сразу за ч/ч
                        wsNew.Cells(outRow, 11).Value = currentExtra(10)
                        
                        ' ----------------------------------------------------------------------
                        ' ТЗ: ВНЕДРЕНИЕ АНАЛИТИЧЕСКИХ СТАТУСОВ В ГРАФУ L ("Дельта" - Столбец 12)
                        ' ----------------------------------------------------------------------
                        ' Условие 2: Если Факт объема (I) равен нулю — ставим минус "-"
                        If currentExtra(5) = 0 Then
                            wsNew.Cells(outRow, 12).Value = "-"
                        ' Условие 3: Если Факт объема (I) больше Исходного объема (G) — пишем "> на: дельта"
                        ElseIf currentExtra(5) > currentExtra(3) Then
                            wsNew.Cells(outRow, 12).Value = "> на: " & Format(currentExtra(5) - currentExtra(3), "#,##0.00")
                        ' Условие 1: Если Исходный (G) больше Факта (I) и Факт не равен нулю — пишем "В работе"
                        Else
                            wsNew.Cells(outRow, 12).Value = "В работе"
                        End If
                        ' ----------------------------------------------------------------------
                        
                        ' Проверка дат через IsDate для Уровня 3 (сдвинуты на столбцы M (13) и N (14))
                        If IsDate(currentExtra(7)) And currentExtra(7) <> 0 And currentExtra(7) <> "00.01.1900" Then
                            wsNew.Cells(outRow, 13).Value = CDate(currentExtra(7))
                        Else
                            wsNew.Cells(outRow, 13).Value = 0
                        End If
                        
                        If IsDate(currentExtra(8)) And currentExtra(8) <> 0 And currentExtra(8) <> "00.01.1900" Then
                            wsNew.Cells(outRow, 14).Value = CDate(currentExtra(8))
                        Else
                            wsNew.Cells(outRow, 14).Value = 0
                        End If
                        
                        ' Процент готовности уходит в столбец O (15)
                        If currentExtra(3) > 0 Then
                            pL3 = currentExtra(5) / currentExtra(3)
                            If pL3 > 1# Then pL3 = 1#
                            wsNew.Cells(outRow, 15).Value = pL3
                        Else
                            wsNew.Cells(outRow, 15).Value = 0#
                        End If
                        
                        Set rngL3 = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 15)) ' Расширено до O
            ...
            If SortRow <= UBound(arrL1) Then
                ...
                Set rngBlankRow = wsNew.Range(wsNew.Cells(outRow, 2), wsNew.Cells(outRow, 15)) ' Расширено до O
...
' БЛОК 4 ИЗ 5 (ЧАСТЬ 1): ВНЕШНЯЯ ГРУППИРОВКА И ОБЩИЙ КОНТУРНЫЙ ОБХОД ДЛЯ УРОВНЯ 3 ПО НОВОЙ СЕТКЕ
    ' === МАРКЕР_ФИНАЛЬНОГО_ОФОРМЛЕНИЯ_START ===
    Dim totalRows As Long, currentGroupStart As Long
    Dim totalTargetHours As Double
    Dim totalSpentHours As Double
    Dim k1Var As Variant
    
    totalRows = wsNew.Cells(wsNew.Rows.Count, 4).End(xlUp).Row
    
    ' ПОДСПОРНЫЙ СЧЕТЧИК: Собираем глобальные суммы плана и факта ч/ч по всем Объектам 1 уровня для титула
    totalTargetHours = 0#
    totalSpentHours = 0#
    ...
    If dictL1.Count > 0 Then
        For Each k1Var In dictL1.Keys
            ...
            totalSpentHours = totalSpentHours + dictL1(k1Var)("Spent")
        Next k1Var
    End If
    
    ' 2. Динамическое определение внешних границ групп по пустым строкам
    currentGroupStart = 4
    ...
    ' 3. Нанесение общей базовой сетки тонких границ на всю заполненную таблицу со 2-й строки (расширено до O)
    Dim wholeTable As Range
    Set wholeTable = wsNew.Range("B2:O" & totalRows)
    wholeTable.Borders.LineStyle = xlContinuous
    wholeTable.Borders.Weight = xlThin
    
    ' 4. Поиск пачки строк Уровня 3 и обводка ВСЕГО БЛОКА в жирную рамку по новым координатам (B:O)
    ...
    For i = 4 To totalRows
        ...
        Else
            If blockStartRow <> 0 Then
                wsNew.Range(wsNew.Cells(blockStartRow, 2), wsNew.Cells(i - 1, 15)).BorderAround LineStyle:=xlContinuous, Weight:=xlMedium
                blockStartRow = 0
            End If
        End If
    Next i
    ...
    If blockStartRow <> 0 Then
        wsNew.Range(wsNew.Cells(blockStartRow, 2), wsNew.Cells(totalRows, 15)).BorderAround LineStyle:=xlContinuous, Weight:=xlMedium
    End If
    
    ' 5. Настройка двухъярусной шапки в строках 2 и 3 (расширено до O)
    With wsNew.Range("B2:O3")
        .Font.Name = "Segoe UI"
        .Font.Size = 10
        .Font.Bold = True
    ...
    wsNew.Rows(3).RowHeight = 35
      ...
      ' 6. Геометрия столбцов под новую расширенную координатную сетку
    ...
    With wsNew.Range("F:H"): .ColumnWidth = 11: .HorizontalAlignment = xlCenter: .VerticalAlignment = xlCenter: End With
    With wsNew.Range("I:K"): .ColumnWidth = 11: .HorizontalAlignment = xlCenter: .VerticalAlignment = xlCenter: End With
    With wsNew.Columns("L"): .ColumnWidth = 15: .HorizontalAlignment = xlCenter: .VerticalAlignment = xlCenter: End With ' ТЗ: Графа L (Дельта) шириной 15
    
    ' ТЗ: Точечно переопределяем ширину граф I и J на 15 единиц для визуального простора
    wsNew.Columns("I").ColumnWidth = 15
    wsNew.Columns("J").ColumnWidth = 15
    
    ' 7. НАСТРОЙКА УРОВНЕЙ ПО ТЗ (Шрифт Segoe UI, ВЕЗДЕ ЖИРНЫЙ, РАЗМЕР СТРОГО 11, ВЫСОТА 48 / 2)
    Dim cellMark As Variant
    Dim valVolG As Double, valVolI As Double
    Dim valPlanHoursH As Double
    ...
    For i = 4 To totalRows
        ...
        Else
            ' Для всех заполненных ячеек устанавливаем жирный Segoe UI 11 на весь рабочий диапазон (B:O)
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 15)).Font.Name = "Segoe UI"
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 15)).Font.Bold = True
            wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 15)).Font.Size = 11
            ...
            If InStr(1, cellMark, ".") = 0 Then
                ...
                ' ==========================================================================
                wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 15)).Font.Italic = True
                wsNew.Cells(i, 4).HorizontalAlignment = xlLeft: wsNew.Cells(i, 4).VerticalAlignment = xlCenter
                
                ' Принудительное Merge плашек E-G
                wsNew.Range(wsNew.Cells(i, 5), wsNew.Cells(i, 7)).Merge
                
                ' Встречное безрамочное выравнивание текста и чисел
                ...
                ' Стираем вертикальные границы на стыках
                wsNew.Cells(i, 8).Borders(xlEdgeLeft).LineStyle = xlNone
                wsNew.Cells(i, 10).Borders(xlEdgeLeft).LineStyle = xlNone
                
                With wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 15)).Borders(xlEdgeBottom)
                    .LineStyle = xlContinuous: .Weight = xlThin: .Color = RGB(0, 0, 0)
                End With
                
                ' Для граф H и J оставляем открытые нули 0 для верхнего яруса
                wsNew.Cells(i, 8).NumberFormat = "#,##0"
                wsNew.Cells(i, 10).NumberFormat = "#,##0"
                
                ' Проверяем реальные часы в графе H (8) для условного скрытия процентов в новом сдвинутом столбце O (15)
                valPlanHoursH = Val(wsNew.Cells(i, 8).Value)
                If valPlanHoursH = 0 Then
                    wsNew.Cells(i, 15).NumberFormat = "0.0%;;""-"""
                Else
                    wsNew.Cells(i, 15).NumberFormat = "0.0%"
                End If
            ...
            ElseIf UBound(Split(cellMark, ".")) = 1 Then
                ...
                ' Для граф H и J оставляем открытые нули 0 для подгрупп
                wsNew.Cells(i, 8).NumberFormat = "#,##0"
                wsNew.Cells(i, 10).NumberFormat = "#,##0"
            ...
            Else
                ...
                ' ==========================================================================
                wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 15)).Font.Italic = False
                wsNew.Cells(i, 4).HorizontalAlignment = xlLeft: wsNew.Cells(i, 4).VerticalAlignment = xlCenter
                
                wsNew.Range(wsNew.Cells(i, 6), wsNew.Cells(i, 7)).NumberFormat = "#,##0.00;;""-"""
                wsNew.Cells(i, 8).NumberFormat = "#,##0;;""-"""
                wsNew.Cells(i, 9).NumberFormat = "#,##0.00;;""-"""
                
                ' Формат для графы J (10) на Уровне 3 восстанавливаем со скрытием нулей минусом "-"
                wsNew.Cells(i, 10).NumberFormat = "#,##0;;""-"""
                wsNew.Cells(i, 11).NumberFormat = "#,##0.00;;""-""" ' Остаток объема (K)
                
                ' Новая графа L ("Дельта" - Столбец 12) по умолчанию центрируется для красоты текстовых статусов
                wsNew.Cells(i, 12).HorizontalAlignment = xlCenter
                wsNew.Cells(i, 12).VerticalAlignment = xlCenter
                
                ' Для Уровня 3 сохраняем двухфакторную проверку по физическим объемам G (7) и I (9) в новом сдвинутом столбце O (15)
                valVolG = Val(wsNew.Cells(i, 7).Value)
                valVolI = Val(wsNew.Cells(i, 9).Value)
                
                If valVolG = 0 And valVolI = 0 Then
                    wsNew.Cells(i, 15).NumberFormat = "0.0%;;""-"""
                Else
                    wsNew.Cells(i, 15).NumberFormat = "0.0%"
                End If
    ...
    Next i
       ...
       ' 8. Форматирование сместившихся столбцов дат M и N (13 и 14) с маскировкой нулевых дат минусом
    With wsNew.Range("M:N")
        .ColumnWidth = 18
        .NumberFormat = "dd.mm.yyyy;;""-""" ' ТЗ: Наличие физического 0 превратится в минус
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
        .WrapText = True
    End With
    
    ' 9. --- НАСТРОЙКА И ВИЗУАЛИЗАЦИЯ СТОЛБЦА O («БАТАРЕЙКИ» ПРОЦЕНТОВ ПО НОВЫМ КООРДИНАТАМ) ---
    Dim rngPercent As Range
    Set rngPercent = wsNew.Range("O4:O" & totalRows)
    
    With wsNew.Columns("O")
        .ColumnWidth = 16
        .HorizontalAlignment = xlCenter
        .VerticalAlignment = xlCenter
    End With
    
    ' Градиентная заливка батареек накладывается поверх наших интеллектуальных форматов в столбце O
    ...
    ' ==========================================================================
    ' 10. ОФОРМЛЕНИЕ ИМЕЮЩЕЙСЯ СТРОКИ 1 ПОД ТИТУЛ С KPI-БЛОКОМ (РАСШИРЕНО ДО B:O)
    ' ==========================================================================
    ...
    Set titleTextCell = wsNew.Range("B1:M1") ' Левый текстовый блок расширен до M из-за новой графы
    Set titleSumCell = wsNew.Range("N1:O1")  ' Правый KPI-блок ч/ч занимает новые крайние ячейки N-O
    
    titleTextCell.Merge
    titleSumCell.Merge
    
    ' Запись многострочного текста титула в левый блок
    ...
    ' ТЗ: Вывод симметричного KPI в головной столбец N (14)
    wsNew.Cells(1, 14).Value = "ПЛАН ТРУДОЗАТРАТ: " & Format(totalTargetHours, "#,##0") & " ч/ч" & vbCrLf & vbCrLf & _
                               "ФАКТ ТРУДОЗАТРАТ: " & Format(totalSpentHours, "#,##0") & " ч/ч"
    ...
    ' Настройка правого KPI-блока (размер шрифта 11, чтобы две строчки свободно дышали в высоте 120)
    With titleSumCell
        .Font.Name = "Times New Roman"
        .Font.Size = 11
        .Font.Bold = True
    ...
    ' Строгие двойные бухгалтерские линии под расширенным титульным блоком (B:O)
    wsNew.Range("B1:O1").Borders(xlEdgeBottom).LineStyle = xlDouble
    wsNew.Range("B1:O1").Borders(xlEdgeBottom).Weight = xlThick
    ...
    MsgBox "Генерация отчета завершена! Новая графа 'Дельта' добавлена, вся сетка пересчитана.", vbInformation
