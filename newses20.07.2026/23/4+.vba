...
' БЛОК 3 ИЗ 5 (ЧАСТЬ 1): ФОРМИРОВАНИЕ ШАПКИ, СТРУКТУРНАЯ СОРТИРОВКА И ВЫГРУЗКА УРОВНЯ 1
    ...
    If dictL1.Count > 0 Then
        ...
        For SortRow = 1 To UBound(arrL1)
            ...
            If metaL1("MinStart") <> DateAdd("yyyy", 100, Date) Then wsNew.Cells(outRow, 12).Value = metaL1("MinStart")
            If metaL1("MaxEnd") <> CDate(0) Then wsNew.Cells(outRow, 13).Value = metaL1("MaxEnd")
            ...
            If subDictL2.Count > 0 Then
                ...
                For r2 = 1 To UBound(arrL2)
                    ...
                    ' ИСПРАВЛЕНИЕ: Возвращаем принудительное объединение граф E, F, G (5, 6, 7) для Уровня 2
                    wsNew.Range(wsNew.Cells(outRow, 5), wsNew.Cells(outRow, 7)).Merge
                    wsNew.Cells(outRow, 5).Value = "Трудозатраты по работам:"
                    
                    ' Значение плановых ч/ч подгруппы падает в одиночную графу H (8) строго над Фактом
                    wsNew.Cells(outRow, 8).Value = metaL2("TargetHours")
                    
                    ' Графа I (9) — текст "Затраты:"
                    wsNew.Cells(outRow, 9).Value = "Затраты:"
                    ...
                    For Each kVol In subDictL3.Keys
                        ...
                        ' Даты сдвинулись в столбцы L (12) и M (13)
                        If currentExtra(7) <> 0 And currentExtra(7) <> "00.01.1900" Then wsNew.Cells(outRow, 12).Value = currentExtra(7)
                        If currentExtra(8) <> 0 And currentExtra(8) <> "00.01.1900" Then wsNew.Cells(outRow, 13).Value = currentExtra(8)
...
' БЛОК 4 ИЗ 5 (ЧАСТЬ 1): ВНЕШНЯЯ ГРУППИРОВКА И ОБЩИЙ КОНТУРНЫЙ ОБХОД ДЛЯ УРОВНЯ 3 ПО НОВОЙ СЕТКЕ
    ...
    For i = 4 To totalRows
        ...
        Else
            ...
            wsNew.Rows(i).RowHeight = 48 ' ТЗ: Высота всех информационных строк строго 48
                ...
                ' Текст в графе D к левой стороне
                wsNew.Cells(i, 4).HorizontalAlignment = xlLeft
                wsNew.Cells(i, 4).VerticalAlignment = xlCenter
                
                ' ЖЕСТКОЕ ИСПРАВЛЕНИЕ БАГА: Повторно принудительно объединяем E-G в цикле оформления
                ...
                ' Растворяем вертикальные границы на стыках Текст-Число (между G-H и I-J)
                wsNew.Cells(i, 8).Borders(xlEdgeLeft).LineStyle = xlNone
                wsNew.Cells(i, 10).Borders(xlEdgeLeft).LineStyle = xlNone
                
                ' Восстановление непрерывности нижней горизонтальной линии без обрубков
            ...
            ElseIf UBound(Split(cellMark, ".")) = 1 Then
                ...
                ' ЖЕСТКОЕ ИСПРАВЛЕНИЕ БАГА: Повторно принудительно объединяем E-G для Уровня 2
                wsNew.Range(wsNew.Cells(i, 5), wsNew.Cells(i, 7)).Merge
                ...
                ' Восстановление непрерывности нижней горизонтальной линии
                With wsNew.Range(wsNew.Cells(i, 2), wsNew.Cells(i, 14)).Borders(xlEdgeBottom)
                    .LineStyle = xlContinuous
                    .Weight = xlThin
                    .Color = RGB(0, 0, 0)
            ...
            Else
                ...
                ' Форматируем объемы (G, I, K) с копейками, человеко-часы (H, J) — целыми числами
                wsNew.Range(wsNew.Cells(i, 6), wsNew.Cells(i, 7)).NumberFormat = "#,##0.00"   ' Норма, Исх. объем
                wsNew.Cells(i, 8).NumberFormat = "#,##0"                                      ' План ч/ч (Закрытая брешь!)
                wsNew.Cells(i, 9).NumberFormat = "#,##0.00"                                   ' Факт объем
                wsNew.Cells(i, 10).NumberFormat = "#,##0"                                     ' Потрачено ч/ч
                wsNew.Cells(i, 11).NumberFormat = "#,##0.00"                                  ' Остаток объем
            End If
        End If
    Next i
    
    ' 8. Форматирование сместившихся столбцов дат L и M (Ширина 18, Центрирование)
    ...
    ' 9. --- НАСТРОЙКА И ВИЗУАЛИЗАЦИЯ СТОЛБЦА N («БАТАРЕЙКИ» ПРОЦЕНТОВ ПО НОВЫМ КООРДИНАТАМ) ---
